...
class ReportController extends Controller
{
    public function index(Request $request): View
    {
        ...
        $totalsByProject = (clone $baseQuery)
        ...
        $totalsByCategory = (clone $baseQuery)
        ...
        $totalsBySubcategory = (clone $baseQuery)
        ...
        $totalsByAuxiliary = (clone $baseQuery)
            ->leftJoin('auxiliaries', 'auxiliaries.id', '=', 'expenses.auxiliary_id')
            ->selectRaw("COALESCE(auxiliaries.name, 'Sin auxiliar') as name, SUM(expenses.total_amount) as total_amount, COUNT(expenses.id) as expenses_count")
            ->groupBy(DB::raw("COALESCE(auxiliaries.name, 'Sin auxiliar')"))
            ->orderByDesc('total_amount')
            ->get();

        $seriesByDate = (clone $baseQuery)
