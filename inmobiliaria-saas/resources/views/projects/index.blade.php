<x-app-layout x-data="crudTable({ flash: {{ \Illuminate\Support\Js::from(session('status')) }} })" x-on:click="handleClick($event)">
    <x-slot name="header">
        <x-page-header title="Proyectos" description="Administra los proyectos de cada empresa y su estado operativo dentro del módulo financiero.">
            @can('create', App\Models\Project::class)
                <button type="button" data-action="create" data-url="{{ route('projects.create') }}" data-title="Nuevo proyecto" class="rounded-2xl bg-stone-900 px-4 py-2 text-sm font-medium text-white transition hover:bg-stone-700">
                    Nuevo proyecto
                </button>
            @endcan
        </x-page-header>
