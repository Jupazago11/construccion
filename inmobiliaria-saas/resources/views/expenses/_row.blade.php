...
<tr
    ...
    data-row-open
    @if ($expense->invoice)
        data-href="{{ route('invoices.show', $expense->invoice) }}"
    @else
    ...
    x-on:click="
        @if ($expense->invoice)
            window.location.assign(@js(route('invoices.show', $expense->invoice)))
        @else
            openModal(@js(route('expenses.edit', $expense)), 'Editar gasto')
        @endif
    "
>
