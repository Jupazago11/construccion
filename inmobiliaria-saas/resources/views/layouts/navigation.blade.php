...
<nav x-data="{ open: false, mastersOpen: {{ request()->routeIs('users.*', 'assets.*', 'assets2.*', 'providers.*', 'providers2.*', 'product-catalog.*', 'activity-catalog.*') ? 'true' : 'false' }} }" class="sticky top-0 z-50 border-b border-stone-200 bg-white sm:bg-white/95 sm:backdrop-blur">
        ...
        <div class="flex h-16 justify-between">
            <div class="flex">
                ...
                <div class="hidden space-x-2 sm:-my-px sm:ms-10 sm:flex sm:items-center">
                    ...
                    @endcan
                    <x-nav-link :href="$navUrl('vehiculo.index')" :active="request()->routeIs('vehiculo.*')">
                        Vehículo
                    </x-nav-link>
    ...
    >
        ...
        <div class="mx-auto max-w-sm space-y-1 px-4 py-4">
            ...
            <a href="{{ $navUrl('vehiculo.index') }}" class="mobile-nav-item {{ request()->routeIs('vehiculo.*') ? 'mobile-nav-item--active' : '' }}">
                <span class="mobile-nav-icon">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.75"><path stroke-linecap="round" stroke-linejoin="round" d="M8 16.5v.75m8-.75v.75M3 12l1.286-4.286A2 2 0 016.2 6.25h11.6a2 2 0 011.914 1.464L21 12m-18 0v5a1 1 0 001 1h1.5a1 1 0 001-1v-1h11v1a1 1 0 001 1H20a1 1 0 001-1v-5m-18 0h18" /></svg>
                </span>
                <span>Vehículo</span>
            </a>
