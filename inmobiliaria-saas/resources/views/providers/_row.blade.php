<tr data-row-id="{{ $provider->id }}">
    ...
    <td class="px-6 py-4">
        <button
            type="button"
            data-action="status"
            data-url="{{ route('providers.status', $provider) }}"
            data-current-status="{{ $provider->status }}"
            data-status-options='@json(["active", "inactive"])'
        >
            <x-status-badge :value="$provider->status" class="cursor-pointer transition hover:opacity-80" />
        </button>
    </td>
    <td class="px-6 py-4">
        <div class="flex items-center justify-end gap-2">
            <button
                type="button"
                data-action="edit"
                data-url="{{ route('providers.edit', $provider) }}"
            ...
            <button
                type="button"
                data-action="delete"
                data-url="{{ route('providers.destroy', $provider) }}"
                data-confirm-message="¿Deseas archivar este proveedor? Solo se permite si no tiene gastos."
