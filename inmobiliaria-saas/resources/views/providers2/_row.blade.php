<tr data-row-id="{{ $provider2->id }}">
    <td class="w-36 whitespace-nowrap px-6 py-4">
        <div class="font-semibold text-stone-900">{{ $provider2->created_at?->format('Y-m-d') ?: 'Sin fecha' }}</div>
    </td>
    <td class="px-6 py-4 text-stone-700">
        <span class="font-semibold text-stone-900">{{ $provider2->name }}</span>
        <span class="mx-2 text-stone-300">·</span>
        <span>{{ $provider2->type?->name ?: 'Sin tipo' }}</span>
        <span class="mx-2 text-stone-300">·</span>
        <span class="text-stone-500">{{ $provider2->document_number ?: 'Sin documento' }}</span>
    </td>
    <td class="px-6 py-4 text-stone-600">
        {{ $provider2->location ?: 'Sin ubicación' }}
    </td>
    <td class="px-6 py-4 text-stone-600">
        {{ $provider2->phone ?: 'Sin teléfono' }}
    </td>
    <td class="px-6 py-4">
        <button
            type="button"
            data-action="status"
            data-url="{{ route('providers2.status', $provider2) }}"
            data-current-status="{{ $provider2->status }}"
            data-status-options='@json(["active", "inactive"])'
        >
            <x-status-badge :value="$provider2->status" class="cursor-pointer transition hover:opacity-80" />
        </button>
    </td>
    <td class="px-6 py-4">
        <div class="flex items-center justify-end gap-2">
            <button
                type="button"
                data-action="edit"
                data-url="{{ route('providers2.edit', $provider2) }}"
            ...
            <button
                type="button"
                data-action="delete"
                data-url="{{ route('providers2.destroy', $provider2) }}"
                data-confirm-message="¿Deseas archivar este Proveedor?"
