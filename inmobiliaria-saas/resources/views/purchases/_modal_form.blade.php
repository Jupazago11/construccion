@php
    $selected = [
        ...
        'is_activity' => (bool) $purchase->activity_id,
...
<form method="POST" action="{{ $action }}" data-ajax-form data-transaction-form data-transaction-type="{{ $payload['transactionType'] ?? 'purchase' }}" class="flex h-full min-h-0 flex-col gap-4 overflow-hidden sm:gap-6">
    ...
    <div class="min-h-0 flex-1 overflow-y-auto pr-1">
        <div class="grid gap-6 md:grid-cols-3">
            ...
            <div>
                <x-input-label :value="'Tipo de registro'" />
                <label class="mt-1 inline-flex w-full items-center gap-3 rounded-2xl border border-stone-200 px-4 py-3 text-sm text-stone-700">
                    <input type="checkbox" name="is_activity" value="1" data-transaction-is-activity class="rounded border-stone-300 text-stone-900 shadow-sm focus:ring-stone-900" @checked($purchase->activity_id)>
                    <span>Es una actividad</span>
                </label>
            </div>

            <div class="relative md:col-span-2" data-transaction-product-wrapper>
            ...
            <div class="relative hidden md:col-span-2" data-transaction-activity-wrapper>
