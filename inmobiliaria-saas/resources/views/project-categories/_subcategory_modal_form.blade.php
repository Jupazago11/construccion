    ...
    <div class="min-h-0 flex-1 overflow-y-auto pr-1">
        <div class="grid gap-6 md:grid-cols-2">
            ...
            <div>
                ...
                <x-text-input id="sort_order" name="sort_order" type="number" min="0" class="mt-1 block w-full" :value="$subcategory->sort_order ?? 0" />
            ...
            <div>
                ...
                <select id="status" name="status" class="mt-1 block w-full rounded-2xl border-stone-300 shadow-sm focus:border-stone-900 focus:ring-stone-900">
                    @foreach (['active' => 'Activo', 'inactive' => 'Inactivo'] as $value => $label)
                        <option value="{{ $value }}" @selected(($subcategory->status ?: 'active') === $value)>{{ $label }}</option>
                    @endforeach
                </select>
