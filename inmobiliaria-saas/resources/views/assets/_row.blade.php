@php
    $assetBaseTotal = (float) $asset->purchase_value * (int) ($asset->quantity ?: 1);
@endphp
...
>
    ...
    <td class="px-6 py-4 text-stone-600">
        {{ number_format((int) ($asset->quantity ?: 1), 0, ',', '.') }}
    </td>
    ...
    <td class="whitespace-nowrap px-6 py-4 text-stone-900">
        ...
        <div class="text-xs text-stone-500">$ {{ number_format((float) $asset->purchase_value, 0, ',', '.') }} x {{ number_format((int) ($asset->quantity ?: 1), 0, ',', '.') }}</div>
    </td>
    <td class="px-6 py-4 text-stone-600">
        <div class="app-two-line-text min-w-52">
            <div>{{ number_format((int) ($asset->active_novelties_count ?? 0)) }} registros</div>
            <div class="text-stone-500">Costo que da valor: $ {{ number_format((float) ($asset->active_novelties_cost_sum ?? 0), 0, ',', '.') }}</div>
        </div>
    </td>
    <td class="px-6 py-4">
