@php
    ...
    $formUid = 'asset2-novelty-' . $asset2->id . '-' . ($novelty->exists ? $novelty->id : 'new') . '-' . substr(md5($action), 0, 8);
@endphp
...
<form
    ...
    x-data="assetTypeManager({
        ...
        initialCompanyId: @js((string) $asset2->company_id),
        entityName: 'novedad',
    })"
    x-init="init()"
    ...
    <div class="min-h-0 flex-1 overflow-y-auto pr-1">
        <div class="space-y-6">
            <div class="rounded-2xl border border-stone-200 bg-stone-50 px-4 py-3 text-sm text-stone-700">
                <div class="font-semibold text-stone-900">{{ $asset2->name }}</div>
            </div>
            ...
            @unless ($novelty->exists)
                <div class="space-y-3">
                    <div class="flex items-center justify-between gap-4">
                        ...
                        <span class="text-xs text-stone-500">{{ $asset2->novelties->count() }} visibles</span>
                    </div>

                    @forelse ($asset2->novelties as $recentNovelty)
                            ...
                            <div class="flex items-center justify-between gap-4">
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <button
                                        type="button"
                                        data-action="edit"
                                        data-url="{{ route('assets2.novelties.edit', ['asset2' => $asset2, 'novelty' => $recentNovelty] + request()->query()) }}"
                                    ...
                                    <button
                                        type="button"
                                        data-action="delete"
                                        data-url="{{ route('assets2.novelties.destroy', ['asset2' => $asset2, 'novelty' => $recentNovelty] + request()->query()) }}"
                    ...
                    @empty
                        <div class="rounded-2xl border border-dashed border-stone-300 bg-stone-50 px-4 py-8 text-center text-sm text-stone-500">
                            Este activo aun no tiene novedades registradas.
                        </div>
                    @endforelse
                </div>
            @endunless
            ...
            <div class="grid gap-6 md:grid-cols-2">
                ...
                <div>
                    ...
                    <x-text-input
                        ...
                        required
                    />
                    <p class="mt-2 text-xs text-stone-500">Escribe solo numeros. El valor se formatea automaticamente con puntos de miles.</p>
                ...
                <div class="md:col-span-2">
                    <x-input-label for="{{ $formUid }}-description" :value="'Descripcion'" />
